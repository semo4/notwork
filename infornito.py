...
def export_profile(profile_id):
    ...
    print('\n==== [{}] {} ({}) ===='.format(profile_id, profile_information['browser'].capitalize(),
            ...
            try:
                ...
                if not os.path.exists(os.path.join(templates_path, 'html')):
                    ...
                    try:
                        ...
                        print('\t[-]' + str(e))
                        print(
                            '\t[!] if you have any problem during download, you can download template from {} and extract it to {}.'.format(
                                html_template_url, templates_path))
                        exit()
                ...
                output_list = []
                for item in history:
                    temp = [html.escape(item['url']), html.escape(str(item['title'])), item['count'],
                            item['last_visit']]
    ...
    if args.id:
        ...
        for (key, profile_information) in enumerate(profiles):
            browser_profile_list.append(
                [key + 1, profile_information['name'], profile_information['browser'].capitalize()])
    ...
    if query_filters != None:
        ...
        if query_filters.get('protocol'):
            downloads = [item for item in downloads if
                         filterer.protocol_equal(item['url'], query_filters.get('protocol'))]
...
# python infornito.py export --profile 2
